...
class LogFilePlot(traits.HasTraits):
    ...
    TOFTimeVariableName = traits.String("TOFTime")
    ...
    plotFitButton = traits.Button("Plot Fit")
    logFile = traits.File(r'\\ursa\AQOGroupFolder\Experiment Humphry\Data\eagleLogs\default.csv')
    ...
    autoLoadButton = traits.Button("auto", desc="attempt to automatically load settings from OneNote page")
    ...
    fileGroup = traitsui.VGroup(
                                ...
                                traitsui.HGroup(traitsui.Item("series",label="series", style="readonly"),traitsui.Item("seriesEdit", show_label=False), traitsui.Item("autoLoadButton", visible_when="ONENOTE_ENABLED", show_label=False)),
    ...
    def loadSettingsFromOneNotePage(self):
        #try to pull x axis , y axis and series if possible
        logFolder,tail = os.path.split(self.logFile )
        if ONENOTE_ENABLED:
            try:
                logger.info("attempting to pull x and y axis from oneNote")
                logName =  tail.strip(".csv")#this is what the name of the page should be in oneNote
                ...
                if logPage is None:
                    logger.info("no OneNote Page for this file. Can't guess the xaxis,yaxis, series etc.")
                    return
                #if we're here we have found a oneNote page....
                (xAxis,yAxis,series) = eagleOneNote.getParametersOutlineValues()
                self.xAxis = xAxis
                self.yAxis = yAxis
                self.series = series
            except Exception as e:
                logger.error("failed to set xAxis, yAxis, series. Error: %s " % e.message)
                
    def _autoLoadButton_fired(self):
        self.loadSettingsFromOneNotePage()
...
#            [self.legend.x,self.legend.y] = self.oldLegendPosition
    ...
    def savePlotAsImage(self, name=None, oneNote=True):
        """save the current plot container as an image file.
        If name is None it will save the file as a png to the default location
        inside the eagleLog folder with the default name. 
        If name is not None, it should be a complete  path (including extension)
        to the location the file should be saved.
        """
        if name is None:#use automatic location
            logFolder,tail = os.path.split(self.logFile )
            logName = tail.strip(".csv")+" - "+str(self.xAxis)+" vs "+str(self.yAxis)
            imageFileName =os.path.join(logFolder, datetime.datetime.today().strftime("%Y-%m-%dT%H%M%S")+"-"+logName+".png")
        else:
            imageFileName = name
        
        #self.masterContainer.bounds = [width, height]
        #TODO maybe make x and y axis text larger for saved images?
        ####
        originals = {"axisTitleFont":self.firstPlot.x_axis.title_font, 
        "tickLabelFont":self.firstPlot.x_axis.tick_label_font,
        "axisLineWeight":self.firstPlot.x_axis.axis_line_weight}
        saved = {"axisTitleFont":"modern 20","tickLabelFont": "modern 14","axisLineWeight":3.0 }
        
        ###change plot settings for better image capture
        self.firstPlot.x_axis.title_font = saved["axisTitleFont"]
        self.firstPlot.y_axis.title_font = saved["axisTitleFont"]
        self.firstPlot.x_axis.tick_label_font = saved["tickLabelFont"]        
        self.firstPlot.y_axis.tick_label_font = saved["tickLabelFont"]        
        self.firstPlot.x_axis.axis_line_weight = saved["axisLineWeight"]        
        self.firstPlot.y_axis.axis_line_weight = saved["axisLineWeight"]
        self.container.padding = (120,80,80,80)
        ####
        [width, height]=self.container.outer_bounds
        self.container.do_layout(force=True)
        gc = chaco.PlotGraphicsContext((width, height), dpi=72)
        gc.render_component(self.container)
        try:
            gc.save(imageFileName)
        except IOError as e:
            errString = "got an IOError when saving file with Kiva GC. Don't know this happens sometimes....gc=%s" % gc
            logger.error(errString)
        finally:
            ###revert plot settings
            self.firstPlot.x_axis.title_font = originals["axisTitleFont"]
            self.firstPlot.y_axis.title_font = originals["axisTitleFont"]
            self.firstPlot.x_axis.tick_label_font = originals["tickLabelFont"]        
            self.firstPlot.y_axis.tick_label_font = originals["tickLabelFont"] 
            self.firstPlot.x_axis.axis_line_weight = originals["axisLineWeight"]        
            self.firstPlot.y_axis.axis_line_weight = originals["axisLineWeight"]
            self.container.padding = (120,20,20,40)
        ...
        if ONENOTE_ENABLED and oneNote:
            try:
                ...
                eagleOneNote.addImageToPlotOutline(imageFileName, (width,height),rewrite=True)
    ...
    def _savePlotButton_fired(self):
        ...
        self.savePlotAsImage()#default location as no name argument passed
    ...
    def _autoSavePlot(self):
        ...
        defaultLocation = os.path.join("\\\\ursa","AQOGroupFolder","Experiment Humphry","Data","eagleLogs","autoSaveEagleLogFilePlot.png")
        self.savePlotAsImage(name=defaultLocation, oneNote=False)
    ...
    def _librarianButton_fired(self):
        librarian = logLibrarianOneNote.Librarian(logFolder = os.path.split(self.logFile)[0], xAxis = self.xAxis, yAxis = self.yAxis)
    ...
    def _deleteSelectedButton_fired(self):
        ...
        try:
            dataframe = pandas.read_csv(self.logFile, index_col="datetime", parse_dates = True)
        except ValueError as e:
            logger.error("No datetime column. Is this a valid Eagle log file? I will try without index")
            dataframe = pandas.read_csv(self.logFile, parse_dates = True)
        #make backup of original file before deleting        
        shutil.copy2(self.logFile, self.logFile+".backup")
    ...
    def plotFit(self):
        ...
        if self.fitPlot is not None:
            self.container.remove(self.fitPlot)
        self.fitXS = chaco.ArrayDataSource(self.logFilePlotFitterReference.dataSets[self.logFilePlotFitterReference.selectedDataSet][0])
        self.fitYS = chaco.ArrayDataSource(self.logFilePlotFitterReference.modelFitResult.best_fit)#ys evaluated at xs with correct fit parameters
