...
defaultDarkImagePath = os.path.join("\\\\ursa","AQOGroupFolder","Experiment Humphry","Experiment Control and Software","experimentEagle","calibration","darkPictures","Andor1","2020 03 30 - dark Andor1 - interval 0_017 temperature -80_0 .npy")
...
class StandardAndor1(processors.Processor):
    ...
    optionsDict = collections.OrderedDict((("process?",True),("darkSubtraction?",True),("rawImageWithDarkSubtraction?", False),
                                          ("rescale?",False),("rescaleInitialX",350),("rescaleInitialY",100),("rescaleWidth",100), ("rescaleHeight",100),("drawRescaleRegion?",True),
                                          ("rotationAngle",-45.0),
                                          ("Skip initial rows",0),
                                          ("Skip final rows",2),
                                          ("Shift Light Picture Vertically by rows",-2),
    ...
    def fastKineticsCrop(self,rawArray,n):
        """ Atom picture is usually on rows 0-512, light picture on rows 512-1023 """
        ...
        pic1 = rawArray[atomStart+skipInitial:atomEnd-skipFinal] # e.g. [0:512] -> 0-511
        pic2 = rawArray[lightStart+skipInitial:lightEnd-skipFinal] # e.g. [512:1024] -> 512-1023
        return pic1, pic2
