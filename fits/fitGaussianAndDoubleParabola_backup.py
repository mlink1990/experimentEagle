...
class GaussianAndDoubleParabolaFit(Fit):
    def __init__(self, **traitsDict):
        ...
        self.variablesList = [self.x0,self.y0,self.dist,self.AGauss,self.sigmax,self.sigmay,self.AParab_1,self.wParabX_1,self.wParabY_1,self.AParab_2,self.wParabX_2,self.wParabY_2,self.B]
...
#        self.summedODAtomNumber_2 = CalculatedParameter(name="Summed OD Atom Number", desc="Using optical density summed in the calculated region we calculate an estimate of atom number. Clipped at 0. (i.e. does not count negatives)")
        ...
        self.summedOD_total = CalculatedParameter(name="Summed OD", desc="Optical density summed in the calculated region. Clipped at 0. (i.e. does not count negatives)")
        self.summedODAtomNumber_total = CalculatedParameter(name="Summed OD Atom Number", desc="Using optical density summed in the calculated region we calculate an estimate of atom number. Clipped at 0. (i.e. does not count negatives)")
        
        self.condensateFraction = CalculatedParameter(name="Condensate Fraction", desc="Condensate fraction, where condensate is calculated from both parabolas")
        ...
        self.calculatedParametersList = [self.NThermal,self.NCondensate_1,self.NCondensate_2, self.NCondensate, self.N]

    @staticmethod
    def fitFunc(positions,x0,y0,dist,AGauss,sigmax,sigmay,AParab_1,wParabX_1,wParabY_1,AParab_2,wParabX_2,wParabY_2,B):
        """ Gaussian + Parabola
...
#        return AGauss*scipy.exp(-(positions[0]-x0)**2/(2.*sigmax**2)-(positions[1]-y0)**2/(2.*sigmay**2))+AParab*((1-((positions[0]-x0)/wParabX)**2-((positions[1]-y0)/wParabY)**2).clip(0)**(1.5))+B
        return AGauss*scipy.exp(-(positions[0]-x0)**2/(2.*sigmax**2)-(positions[1]-y0)**2/(2.*sigmay**2))+AParab_1*(1-((positions[0]-(x0+dist/2.0))/wParabX_1)**2-((positions[1]-y0)/wParabY_1)**2).clip(0)**(1.5)+AParab_2*(1-((positions[0]-(x0-dist)/2.0)/wParabX_2)**2-((positions[1]-y0)/wParabY_2)**2).clip(0)**(1.5) + B
    ...
    def _deriveCalculatedParameters(self):
        ...
        NCondensate_1=(parabolaIntegral_1*imagePixelArea)*(1.0+4.0*self.physics.imagingDetuningLinewidths**2)/self.physics.selectedElement.crossSectionSigmaPlus
        ...
        NCondensate_2=(parabolaIntegral_2*imagePixelArea)*(1.0+4.0*self.physics.imagingDetuningLinewidths**2)/self.physics.selectedElement.crossSectionSigmaPlus
    ...
    def _getIntelligentInitialValues(self):
        ...
        dist = maxIndex_2[1] - maxIndex_1[1]
        ...
        p0=[x0,y0,dist,AGauss,sigmax,sigmay,AParab_1,wParabX_1,wParabY_1, AParab_2, wParabX_2, wParabY_2,B]
