# -*- coding: utf-8 -*-
"""
Created on Sun Jan 17 14:01:31 2016
...
def integrand(z,x,y,A, x0,sigmax,y0,sigmay):
    return A*scipy.exp(-scipy.absolute( (x-x0)/(2.*sigmax)-(y-y0)/(2.*sigmay)-(z)/(2.*sigmax) )) 

def performIntegral(x,y, A,x0,sigmax,y0,sigmay):
    return scipy.integrate.quad(integrand,-1000,1000, args=(x,y,A, x0,sigmax,y0,sigmay))[0]

vectorisedIntegral = scipy.vectorize(performIntegral)

class ClippedExponentialIntegral(Fit):
    """Sub class of Fit which implements a saturated exponential integral  """
    def __init__(self, **traitsDict):
        super(ClippedExponentialIntegral, self).__init__(**traitsDict)
        self.function = "(A*exp(-(x-x0)/(2.*sigmax)-(y-y0)/(2.*sigmay))+B)"
        self.name = "Saturated Exponential Integral"
        ...
        self.clipValue = Parameter(name="clipValue")
        
        self.variablesList = [self.A,self.x0,self.sigmax,self.y0,self.sigmay,self.B, self.clipValue ]
        ...
        self.calculatedParametersList = [self.N,self.stdevX,self.stdevY, self.Tx,self.Ty , self.T,self.TxHarmonic,self.TyHarmonic,self.THarmonic, 
                                         self.aspectRatio,self.summedOD,self.summedODAtomNumber, self.criticalTemperature,self.thermalDensity,
                                         self.thermalDensityHarmonic,self.phaseSpaceDensity,self.phaseSpaceDensityHarmonic,self.vbarLinear,self.collisionTimeLinear]
    ...
    def fitFunc(positions, A,x0,sigmax,y0,sigmay,B,clipValue):
        ...
        return scipy.clip(vectorisedIntegral(positions[0],positions[1],A, x0,sigmax,y0,sigmay),0,clipValue)
    ...
    def _deriveCalculatedParameters(self):
        ...
        logger.info("TxHarmonic = %G" % TxHarmonic )
        ...
        logger.info("TyHarmonic = %G" % TyHarmonic )        
        ...
        logger.info("criticalTemperature = %G" % Tc )   
