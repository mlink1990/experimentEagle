...
configFile = os.path.join("config","fitSettings","fits.json")
...
class FitThread(threading.Thread):
    ...
    def run(self):
        logger.warning("starting fitting thread")
        if self.fitReference.conditionalFitBool:
            if hasattr(self.fitReference.physics,"variables"):
                if "eagleID" in self.fitReference.physics.variables:
                    if self.fitReference.physics.variables["eagleID"] != self.fitReference.conditionalFitID:
                        self.fitReference.fittingStatus = "Wrong eagleID, skip fit.."
                        return
                    else:
                        logger.warning("Correct eagleID, continue to fit")
                else:
                    logger.warning("Cannot test conditional fit, because variable 'eagleID' is missing")
            else:
                logger.warning("variables not yet initialized")
        else:
            logger.warning("conditionalFitBool not True")
        try:
            if self.fitReference.autoGuessBool:
                logger.info("auto guess bool is true so do a new guess before fitting")
                self.fitReference._intelligentInitialValues()
            ...
            if self.fitReference.autoPreviousBool:
                logger.info("auto previous bool is true so replacing guess values with calculated values to increase speed of next fit")
...
class Fit(traits.HasTraits):
    ...
    autoDrawBool = traits.Bool(True, desc = "Once a fit is complete update the drawing of the fit or draw the fit for the first time")
    autoGuessBool = traits.Bool(False, desc = "Perform a new guess before fitting")
    ...
    autoPreviousBool = traits.Bool(False, desc = "Whenever a fit is completed replace the guess values with the calculated values (useful for increasing speed of the next fit)")
    ...
    logToNas = traits.Bool(True, desc = "If true, log goes to Humphry-NAS instead of ursa")
##    logDirectory = os.path.join("\\\\ursa","AQOGroupFolder","Experiment Humphry","Data","eagleLogs")
    logDirectory = os.path.join("G:",os.sep,"Experiment Humphry","Data","eagleLogs")
    logDirectoryNas = os.path.join("\\\\192.168.16.71","Humphry","Data","eagleLogs")
    ...
    conditionalFitBool = traits.Bool(False, desc = "If true, fit is only executed, if current sequence contains matching variable 'eagleID'")
    ...
    buttons = traitsui.HGroup(
        traitsui.VGroup(
            ...
            traitsui.HGroup(traitsui.Item("autoPreviousBool", label="Auto previous?", resizable=True),traitsui.Item("usePreviousFitValuesButton", show_label=False, resizable=True)),
            show_border=True
        ),
        traitsui.VGroup(
            ...
            traitsui.HGroup(traitsui.Item("conditionalFitBool",label="Conditional Fit?", resizable=True),traitsui.Item("conditionalFitID",label="Fit ID", resizable=True)),
            show_border=True
        )
    )
    ...
    logGroup = traitsui.VGroup(
                traitsui.HGroup(
                    ...
                    traitsui.Item("chooseVariablesButtons", show_label=False, resizable=True, enabled_when="not logAllVariables")
                    ),
    ...
    def __init__(self, **traitsDict):
        ...
        with open(configFile, 'r') as f:
            ...
            if 'logAnalyserBool' in settings:  self.logAnalyserBool = settings['logAnalyserBool']
    ...
    def _log_fit(self):
        ...
        try:
            ...
            if self.imageInspectorReference.selectedFile.endswith("_X2.tif"):
                shutil.copy(self.imageInspectorReference.selectedFile.replace("_X2.tif","_X1.tif"), imagesFolder)
        except IOError as e:
    ...
    def _startX_changed(self):
        ...
        self.imageInspectorReference.ROIPolyPlot.index = chaco.ArrayDataSource( [self.startX,self.endX,self.endX,self.startX] )
        self.imageInspectorReference.ROIPolyPlot.value = chaco.ArrayDataSource( [self.startY,self.startY,self.endY,self.endY] )
